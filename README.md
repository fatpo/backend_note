* [情景一：Java正在运行的进程CPU调优](#java正在运行的进程cpu调优)
* [情景二：Python正在运行的进程CPU调优](#python正在运行的进程cpu调优)
* [情景三：服务访问不了或异常慢卡](#服务访问不了或异常慢卡)
...
```
## 分支3： SSH登录下服务器，观察打命令正常
说明整个服务器没啥问题，那么分2个角度去看“服务访问异常”的情况

### 步骤3.1： 该服务进程存在
这个主要通过日志去查找，如果很久没更新过代码，突然出了问题，可能是第三方服务如七牛、融云等是否异常。

### 步骤3.2： 该服务进程不存在
重启还失败了，很可能硬盘满了，写日志失败。
解决方案：删除多余的文件如远古备份、远古日志空出硬盘。
